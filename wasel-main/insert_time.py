...
{(() => {
  ...
  const supervisorTime = focusedOrder.delivery_time || ${focusedOrder.preferred_delivery_date || ''} .trim();
  ...
  return (
    <p className="flex items-center gap-1 mt-2 text-[#92400E] bg-[#FEF3C7] p-2 rounded-lg text-xs font-bold border border-[#FDE68A]">
      ...
      {userTime ? المطلوب:  : المتوقع من المشرف: }
    </p>
  );
})()}
"""
...
{(() => {
  ...
  const supervisorTime = order.delivery_time || ${order.preferred_delivery_date || ''} .trim();
  const timeToShow = userTime || supervisorTime;
  if (!timeToShow) return null;
  return (
    <p className="flex items-center gap-1 mt-2 text-[#92400E] bg-[#FEF3C7] p-2 rounded-lg text-[10px] font-bold border border-[#FDE68A] w-max">
...
print("Updated DriverPanel with delivery time")
